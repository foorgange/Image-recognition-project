...
> 注：此项目为很早之前技术栈薄弱时出于学习 YOLO 的目的配合 AI 制作的小项目，已很久不维护，未经过严谨测试，代码仅供学习和参考。
...
下面是一个典型的安装 + 启动流程（假设你使用 venv 或者其他虚拟环境）：
...
1. 打开浏览器，访问本地 Streamlit 页面
2. 在页面中选择"上传图片"或"启用摄像头流"
3. 上传 / 切换输入源后，页面会显示带有检测框的图片 / 视频帧
4. 可调整置信度（confidence threshold）等参数，以控制检测灵敏度
5. 可通过提供按钮或脚本保存检测结果（例如截图）
...
`config/` 目录用于存放模型配置（如 YOLO 的 yaml 文件）、阈值、类别名称等。
